...
def vol_cal(ret, cha_name, ret_freq_use: list):
    ...
    volatility = daily_returns.groupby(daily_returns.index).std()
    volatility = volatility.where(daily_returns.groupby(daily_returns.index).count() >= 18)

    volatility.columns = [f"{col}_{cha_name}" for col in volatility.columns]
    volatility.index.name = 'Year_Month'
...
def cha_main(ret, cha_name, ret_freq_use: list):
    ...
    # Calculate the stock characteristics (volatility)
